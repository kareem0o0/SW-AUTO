...
public sealed class Part
{
    ...
    public string Create_stator_sheet(string outFolder, bool closeAfterCreate = false)
    {
        ...
        try
        {
            ...
            string vaultedPath = _pdm.SaveAsPdm(swModel, outFolder);

            // 2. Log the actual vaulted name (e.g., PRT-1001.SLDPRT)
            Console.WriteLine($"Part saved to PDM: {vaultedPath}");
            ...
            // Return the actual name/path created by PDM instead of the hardcoded string
            return Path.GetFileName(vaultedPath); 
        }
        catch (Exception ex)
        {
    ...
    public string CreateSkeleton(double sideOffset, double groundOffset, string outFolder, bool closeAfterCreate = false)
    {
        ...
        Console.WriteLine($"Reference-plane part vaulted at: {vaultedPath}");
        ...
        // 2. Return the actual filename generated by PDM (e.g., "B00105.SLDPRT") 
        // instead of the hardcoded "skeleton.SLDPRT"
        return Path.GetFileName(vaultedPath);
    }
