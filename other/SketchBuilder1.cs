...
public sealed class SketchBuilder
{
    ...
    public void BeginPartSketch(string partName, string outputFolder, SketchPlaneName sketchPlane = SketchPlaneName.Front)
    {
        ...
        // Select requested base plane directly using SelectByID2
        string planeName = sketchPlane == SketchPlaneName.Top ? "Top Plane" :
                           sketchPlane == SketchPlaneName.Right || sketchPlane == SketchPlaneName.Side ? "Right Plane" : "Front Plane";
        bool planeSelected = _activeModel.Extension.SelectByID2(planeName, "PLANE", 0, 0, 0, false, 0, null, 0);
    ...
    public void BeginSketch(SketchPlaneName sketchPlane = SketchPlaneName.Front, ModelDoc2 partModel = null)
    {
        ...
        string planeName = sketchPlane == SketchPlaneName.Top ? "Top Plane" :
                           sketchPlane == SketchPlaneName.Right || sketchPlane == SketchPlaneName.Side ? "Right Plane" : "Front Plane";
        bool selected = _activeModel.Extension.SelectByID2(planeName, "PLANE", 0, 0, 0, false, 0, null, 0);
        if (!selected)
        {
    ...
    public void BeginSketch(SketchEntityReference sketchTargetReference, ModelDoc2 partModel = null)
    {
        ...
        bool selected = _activeModel.Extension.SelectByID2(
            ...
            sketchTargetReference.XMm / 1000.0,
            sketchTargetReference.YMm / 1000.0,
            sketchTargetReference.ZMm / 1000.0,
            false, 0, null, 0);
    ...
    // The SelectPlaneTarget method was extracted and moved to the commented section at end of file

    // Create rectangle entity using selected rectangle type.
    // Coordinates are in mm in sketch space. Z is not used (sketch-local Z is always 0).
    public void CreateRectangle(SketchRectangleType rectangleType, double x1Mm, double y1Mm, double x2Mm, double y2Mm)
    {
        ...
        Console.WriteLine($"CreateRectangle DEBUG: corners mm: ({x1Mm},{y1Mm}) to ({x2Mm},{y2Mm})");

        // Create temporary debug points at corners to verify sketch coordinates
        try
        {
            CreatePoint(x1Mm, y1Mm);
            CreatePoint(x2Mm, y2Mm);
            Console.WriteLine("CreateRectangle DEBUG: corner points created successfully");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"CreateRectangle DEBUG: failed to create corner points: {ex.Message}");
        }

        int beforeCount = GetActiveSketchSegmentCount();

        // Build rectangle by requested variant.
        // For sketch-based rectangle creation the Z coordinate is always sketch-local (0).
        object rectangleResult = null;
        try
        {
            ...
            {
                Console.WriteLine("CreateRectangle DEBUG: API returned null");
            }
            else
            {
                Console.WriteLine("CreateRectangle DEBUG: API returned object successfully");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"CreateRectangle DEBUG: API call error: {ex.GetType().Name}: {ex.Message}");
        }
        ...
        // or when no new segments were committed. Restore the fallback so callers
        // still get a rectangle when the high-level API intermittently fails.
        if (rectangleResult == null || (beforeCount >= 0 && afterCount <= beforeCount))
        {
            Console.WriteLine($"CreateRectangle DEBUG: fallback triggered (result={rectangleResult}, before={beforeCount}, after={afterCount}) — falling back to CreateRectangleByLines().");
    ...
    // Z coordinate is not used for circle creation (sketch-local Z is always 0).
    ...
    public void CreateLine(SketchLineType lineType, double startXmm, double startYmm, double endXmm, double endYmm)
    {
        ...
        switch (lineType)
        {
            ...
            case SketchLineType.Construction:
                ...
                _activeModel.ClearSelection2(true);
                _activeModel.Extension.SelectByID2(string.Empty, "SKETCHSEGMENT", (startX + endX) / 2.0, (startY + endY) / 2.0, 0, false, 0, null, 0);
    ...
    public void MirrorSketchEntities(SketchEntityReference mirrorEntity, params SketchEntityReference[] entitiesToMirror)
    {
        ...
        for (int i = 0; i < entitiesToMirror.Length; i++)
        {
            ...
            bool selected = _activeModel.Extension.SelectByID2(entitiesToMirror[i].Name ?? string.Empty, entitiesToMirror[i].Type, entitiesToMirror[i].XMm / 1000.0, entitiesToMirror[i].YMm / 1000.0, entitiesToMirror[i].ZMm / 1000.0, i > 0, 0, null, 0);
            if (!selected)
            {
                _activeModel.ClearSelection2(true);
        ...
        bool mirrorSelected = _activeModel.Extension.SelectByID2(mirrorEntity.Name ?? string.Empty, mirrorEntity.Type, mirrorEntity.XMm / 1000.0, mirrorEntity.YMm / 1000.0, mirrorEntity.ZMm / 1000.0, true, 0, null, 0);
        if (!mirrorSelected)
        {
            _activeModel.ClearSelection2(true);
        ...
        params SketchEntityReference[] seedEntities)
    {
        ...
        for (int i = 0; i < seedEntities.Length; i++)
        {
            ...
            bool selected = _activeModel.Extension.SelectByID2(seedEntities[i].Name ?? string.Empty, seedEntities[i].Type, seedEntities[i].XMm / 1000.0, seedEntities[i].YMm / 1000.0, seedEntities[i].ZMm / 1000.0, i > 0, 0, null, 0);
            if (!selected)
            {
                _activeModel.ClearSelection2(true);
        ...
        bool constrainedCorners = true)
    {
        ...
        _activeModel.ClearSelection2(true);
        _activeModel.Extension.SelectByID2(firstEntity.Name ?? string.Empty, firstEntity.Type, firstEntity.XMm / 1000.0, firstEntity.YMm / 1000.0, firstEntity.ZMm / 1000.0, false, 0, null, 0);
        if (secondEntity.IsValid())
        {
            _activeModel.Extension.SelectByID2(secondEntity.Name ?? string.Empty, secondEntity.Type, secondEntity.XMm / 1000.0, secondEntity.YMm / 1000.0, secondEntity.ZMm / 1000.0, true, 0, null, 0);
        }
        ...
        SketchEntityReference secondEntity = default)
    {
        ...
        _activeModel.ClearSelection2(true);
        bool firstSelected = _activeModel.Extension.SelectByID2(firstEntity.Name ?? string.Empty, firstEntity.Type, firstEntity.XMm / 1000.0, firstEntity.YMm / 1000.0, firstEntity.ZMm / 1000.0, false, 0, null, 0);
        if (!firstSelected)
        {
        ...
        if (secondEntity.IsValid())
        {
            bool secondSelected = _activeModel.Extension.SelectByID2(secondEntity.Name ?? string.Empty, secondEntity.Type, secondEntity.XMm / 1000.0, secondEntity.YMm / 1000.0, secondEntity.ZMm / 1000.0, true, 0, null, 0);
            if (!secondSelected)
            {
    ...
    // Trim sketch geometry at the provided sketch-space coordinate (mm).
    // You must provide the sketch Z coordinate in millimeters via `zMm`.
    // The trim operation is forwarded to the SOLIDWORKS SketchManager API.
    // `trimMode` is an API-specific integer token (see examples below).
    // Trim sketch geometry at the provided sketch-space coordinate (mm).
    // If the sketch lies on a plane offset from the model origin, provide
    // the sketch Z coordinate in millimeters via `zMm` so the pick is correct.
    // The trim operation is forwarded to the SOLIDWORKS SketchManager API.
    // trimMode is an API-specific integer token (ex: 0 for default trim mode).
    public void CreateSketchTrim(double xMm, double yMm, double zMm = double.NaN, int trimMode = 0)
    {
        EnsureSketchOpen();

        // Determine Z coordinate (meters). If omitted, default to 0.
        double zMeters = double.IsNaN(zMm) ? 0.0 : zMm / 1000.0;

        dynamic skMgr = _activeModel.SketchManager;

        // For modes that require a single selected sketch segment (Closest=0, EntityPoint=3),
        // attempt to select the segment at the provided pick point before calling SketchTrim.
        // For other modes, rely on caller pre-selections.
        bool autoSelected = false;
        if (trimMode == 0 || trimMode == 3)
        {
            _activeModel.ClearSelection2(true);
            autoSelected = _activeModel.Extension.SelectByID2(string.Empty, "SKETCHSEGMENT", xMm / 1000.0, yMm / 1000.0, zMeters, false, 0, null, 0);
        }

        try
        {
            skMgr.SketchTrim(trimMode, xMm / 1000.0, yMm / 1000.0, zMeters);
        }
        finally
        {
            if (autoSelected)
    ...
    // Note: CreateSketchPowerTrim removed — use `CreateSketchTrim` with the
    // appropriate `trimMode` and explicit `zMm` instead.

    // Apply typed sketch relation by selecting two entities using XY coordinates.
    // Usage: ApplySketchRelation(x1Mm, y1Mm, x2Mm, y2Mm, SketchRelationType.Horizontal);
    public void ApplySketchRelation(SketchRelationType relationType, double firstXmm, double firstYmm, double secondXmm , double secondYmm )
    {
        ...
        bool firstSelected = _activeModel.Extension.SelectByID2(string.Empty, "SKETCHPOINT", firstXmm / 1000.0, firstYmm / 1000.0, 0, false, 0, null, 0);
        if (!firstSelected)
        {
            throw new InvalidOperationException("Failed to select first entity for relation at (" + firstXmm + ", " + firstYmm + ") mm.");
        }

        bool secondSelected = _activeModel.Extension.SelectByID2(string.Empty, "SKETCHPOINT", secondXmm / 1000.0, secondYmm / 1000.0, 0, true, 0, null, 0);
        if (!secondSelected)
        {
            _activeModel.ClearSelection2(true);
            throw new InvalidOperationException("Failed to select second entity for relation at (" + secondXmm + ", " + secondYmm + ") mm.");
        }
    ...
    // Backward-compatible overload: keep the original method accepting explicit references.
    public void ApplySketchRelation(SketchRelationType relationType, params SketchEntityReference[] entities)
    {
        ...
        for (int i = 0; i < entities.Length; i++)
        {
            ...
            _activeModel.Extension.SelectByID2(entities[i].Name ?? string.Empty, entities[i].Type, entities[i].XMm / 1000.0, entities[i].YMm / 1000.0, entities[i].ZMm / 1000.0, i > 0, 0, null, 0);
        }
...
*/
        ...
        double? val = null)
    {
        ...
        bool firstSelected = _activeModel.Extension.SelectByID2(string.Empty, string.IsNullOrWhiteSpace(firstType) ? "SKETCHPOINT" : firstType, firstXmm / 1000.0, firstYmm / 1000.0, 0, false, 0, null, 0);
        if (!firstSelected)
        {
            throw new InvalidOperationException(
        ...
        bool secondSelected = _activeModel.Extension.SelectByID2(string.Empty, string.IsNullOrWhiteSpace(secondType) ? "SKETCHPOINT" : secondType, secondXmm / 1000.0, secondYmm / 1000.0, 0, true, 0, null, 0);
        if (!secondSelected)
        {
            _activeModel.ClearSelection2(true);
            throw new InvalidOperationException(
        ...
        double? val = null)
    {
        ...
        bool selected = _activeModel.Extension.SelectByID2(string.Empty, string.IsNullOrWhiteSpace(entityType) ? "SKETCHSEGMENT" : entityType, entityXmm / 1000.0, entityYmm / 1000.0, 0, false, 0, null, 0);
        if (!selected)
        {
            throw new InvalidOperationException(
    ...
    public void Extrude(double depthMm, bool midPlane = false, bool isCut = false)
    {
        ...
        {
        // Face direction planes generation was removed as it is non-generic.
        }
    }
...
*/
    ...
    // Delete a sketch entity at the provided sketch-space XY coordinate (mm).
    // If `zMm` is omitted, the method probes the active sketch for an existing
    // sketch point to infer the model-space Z (lightweight fallback), then
    // attempts to select a nearby sketch entity and delete it. Returns true
    // when an entity was successfully selected and deleted.
    public bool DeleteSketchEntityAt(double xMm, double yMm, double zMm = double.NaN, string explicitType = "")
    {
        EnsureSketchOpen();

        // Determine zMm if omitted by probing existing sketch points.
        if (double.IsNaN(zMm))
        {
            try
            {
                ...
                {
                    object ptsObj = activeSketch.GetSketchPoints();
                    ...
                    if (pts != null && pts.Length > 0)
                    {
                        for (int i = 0; i < pts.Length; i++)
                        {
                            ...
                            try
                            {
                                ...
                                try
                                {
                                    coords = dsp.GetCoords3();
                                }
                                catch
                                {
                                    try
                                    {
                                        coords = new double[] { (double)dsp.X, (double)dsp.Y, (double)dsp.Z };
                                    }
                                    catch
                                    {
                                        coords = null;
            ...
            if (double.IsNaN(zMm)) zMm = 0.0;
        }

        // Try selecting the entity. Prefer explicitType when provided, else
        // try common sketch entity types in a best-effort order.
        _activeModel.ClearSelection2(true);

        bool selected = false;
        if (!string.IsNullOrWhiteSpace(explicitType))
        {
            selected = _activeModel.Extension.SelectByID2(string.Empty, explicitType, xMm / 1000.0, yMm / 1000.0, zMm / 1000.0, false, 0, null, 0);
        }
        else
        {
            string[] fallbackTypes = new[] { "SKETCHSEGMENT", "SKETCHARC", "SKETCHPOINT" };
            for (int i = 0; i < fallbackTypes.Length && !selected; i++)
            {
                selected = _activeModel.Extension.SelectByID2(string.Empty, fallbackTypes[i], xMm / 1000.0, yMm / 1000.0, zMm / 1000.0, false, 0, null, 0);
            }
        }
        ...
        // Attempt delete of selected entity.
        try
        {
